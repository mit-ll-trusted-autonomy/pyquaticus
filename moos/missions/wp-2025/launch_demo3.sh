...
./launch_surveyor.sh -vt -r2 $RVARGS --start=$THO_POS --role=ATTACK_E
echo "Launching Walter Red-Three"
./launch_surveyor.sh -vw -r3 $RVARGS --start=$WAL_POS --role=DEFEND_E
...
./launch_surveyor.sh -vv -b2 $BVARGS --start=$VAL_POS --role=ATTACK_MED
echo "Launching Xavier Blue-Three"
./launch_surveyor.sh -vx -b3 $BVARGS --start=$XAV_POS --role=DEFEND_MED 
cd ..
