...
class RandPolicy(Policy):
    ...
    def compute_actions(self,
                        ...
                        episodes=None,
                        **kwargs):
        return [-1 for _ in obs_batch], [], {}#[self.action_space.sample() for _ in obs_batch], [], {}
...
if __name__ == '__main__':
    ...
    reward_config = {0:rew.sparse, 1:None}#rew.custom_v1, 2:None, 3:None} # Example Reward Config
    ...
    config_dict["flag_keepout"] = 1.5
#config_dict["max_speed"]

    #env = pyquaticus_v0.PyQuaticusEnv(team_size=1, config_dict=config_dict,render_mode='human')

    env_creator = lambda config: pyquaticus_v0.PyQuaticusEnv(team_size=1, config_dict=config_dict,render_mode=None)
#pyquaticus_v0.PyQuaticusEnv(render_mode=RENDER_MODE, reward_config=reward_config, team_size=2)
    env = ParallelPettingZooWrapper(pyquaticus_v0.PyQuaticusEnv(team_size=1, config_dict=config_dict,render_mode=None))#pyquaticus_v0.PyQuaticusEnv(render_mode=RENDER_MODE, reward_config=reward_config, team_size=2))
    ...
    policies = {'agent-0-policy':(None, obs_space, act_space, {}), 
                'nothing':(RandPolicy, obs_space, act_space, {}), 
    ...
    env.close()
    ppo_config = PPOConfig().environment(env='pyquaticus').rollouts(num_rollout_workers=100).resources(num_cpus_per_worker=0.14, num_gpus=1)#int(os.environ.get("RLLIB_NUM_GPUS", "0")))
